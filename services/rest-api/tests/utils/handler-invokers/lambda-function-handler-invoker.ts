...
export class LambdaFunctionHandlerInvoker<TEvent = any, TResult = any> {
  ...
  constructor(
    private readonly config: LambdaFunctionHandlerInvokerConfig<
      TEvent,
      TResult
    >,
  ) {
  ...
  async invokeRemote(event: TEvent) {
    ...
    if (result.FunctionError) {
      throw new Error(
        `FunctionError returned from ${this.config.lambdaFunctionName}. Error details: ${result.Payload}`,
      );
    }
  ...
  async invoke(event: TEvent) {
    return this.invocationMode === InvocationMode.REMOTE_TRIGGER
      ? this.invokeRemote(event)
      : this.invokeLocal(event);
  }
}
