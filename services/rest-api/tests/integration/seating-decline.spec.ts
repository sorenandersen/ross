import { handler } from '@svc/handlers/http/seatings/decline';
...
const API_PATH_TEMPLATE =
  '/restaurants/{restaurantId}/seatings/{seatingId}/decline';
const TEST_DATA_PREFIX = 'declineSeatingTest';
...
describe('`PATCH /restaurants/{restaurantId}/seatings/{seatingId}/decline`', () => {
  ...
  it('returns 204 No Content when declining a seating currently in status PENDING', async () => {
    // **
    // Arrange
    // **
    ...
    // Get seating from DB and verify that status has been updated to DECLINED
    const savedSeating = await getSeating(testSeating.id, testRestaurant.id);
    expect(savedSeating).toBeTruthy();
    expect(savedSeating!.status).toEqual(SeatingStatus.DECLINED);
  });

  it('returns 204 No Content when declining a seating currently in status DECLINED', async () => {
    // **
    // Arrange
    // **
    ...
    // Get seating from DB and verify that status is still DECLINED
    const savedSeating = await getSeating(testSeating.id, testRestaurant.id);
    expect(savedSeating).toBeTruthy();
    expect(savedSeating!.status).toEqual(initialAndExpectedStatus);
  });

  it('returns 409 Conflict when declining a seating currently in status CANCELLED', async () => {
    // **
    // Arrange
    // **
  ...
  it('returns 409 Conflict when declining a seating currently in status ACCEPTED', async () => {
    // **
    // Arrange
    // **
  ...
  it('returns 409 Conflict when declining a seating currently in status SEATED', async () => {
    // **
    // Arrange
    // **
  ...
  it('returns 409 Conflict when declining a seating currently in status CLOSED', async () => {
    // **
    // Arrange
    // **
  ...
  it('returns 403 Forbidden when attempting decline of a seating belonging to another restaurant', async () => {
    // **
    // Arrange
    // **
  ...
  it('returns 404 Not Found when attempting decline of a seating that does not exist', async () => {
    // **
    // Arrange
    // **
    const initialAndExpectedStatus = SeatingStatus.PENDING;
