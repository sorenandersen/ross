...
/**
 * Updates seating status to DECLINED
 */
export const handler = wrap(async (event) => {
  ...
  switch (seating.status) {
    case SeatingStatus.PENDING:
      // Seating meets the requirement for being declined
      // Perform update
      await updateSeatingStatus(
        seatingId,
        restaurantId,
        SeatingStatus.DECLINED,
      );
      break;
    case SeatingStatus.DECLINED:
      // Seating already declined - no actions needed
      break;
    default:
